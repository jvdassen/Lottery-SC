...
contract Oracle {
	...
	function reveal(bytes32 revealHash) public {
		...
		uint256 random = convert(keccak256(abi.encodePacked(blockHash,revealHash)))%max;
		emit RevealHash(msg.sender,revealHash,random);
	}
	event RevealHash(address sender, bytes32 revealHash, uint256 random);
  
    function getRandomNumberOfLength(uint length) public returns (uint) {
        // return some random integer of length 
    }
}
